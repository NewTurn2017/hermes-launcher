...
mod tests {
    ...
    fn parses_step_event() {
        let ev = parse_line(
            r#"{"event":"step","step":"install-hermes","progress":42,"msg":"cloning repo"}"#,
        )
        .unwrap();
        assert_eq!(
            ev,
            HelperEvent::Step {
                step: Step::InstallHermes,
                progress: 42,
                msg: "cloning repo".into()
            }
        );
    }
    ...
    fn parses_codex_variants() {
        assert_eq!(
            parse_line(r#"{"event":"codex_authed","email":"u@example.com"}"#).unwrap(),
            HelperEvent::CodexAuthed {
                email: Some("u@example.com".into())
            }
        );
        assert_eq!(
            parse_line(r#"{"event":"codex_authed","email":null}"#).unwrap(),
            HelperEvent::CodexAuthed { email: None }
        );
        assert_eq!(
            parse_line(r#"{"event":"codex_aborted"}"#).unwrap(),
            HelperEvent::CodexAborted
        );
        assert_eq!(
            parse_line(r#"{"event":"codex_timeout"}"#).unwrap(),
            HelperEvent::CodexTimeout
        );
        assert_eq!(
            parse_line(r#"{"event":"codex_error","detail":"no subscription"}"#).unwrap(),
            HelperEvent::CodexError {
                detail: "no subscription".into()
            }
        );
    }
    ...
    fn parses_done_and_error() {
        assert_eq!(
            parse_line(r#"{"event":"done","step":"verify","ok":true}"#).unwrap(),
            HelperEvent::Done {
                step: Step::Verify,
                ok: true
            }
        ...
        assert_eq!(
            parse_line(
                r#"{"event":"error","step":"install-hermes","level":"fatal","detail":"boom"}"#
            )
            .unwrap(),
            HelperEvent::Error {
                step: Step::InstallHermes,
